...
import com.berger.bergerXpressVisualiserPk.models.Size
import com.berger.bergerXpressVisualiserPk.viewHolders.SizesListViewHolder

class SizesListAdapter (val activity: Activity, private val sizes: List<Size>) : androidx.recyclerview.widget.RecyclerView.Adapter<SizesListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: SizesListViewHolder, p1: Int) {
        ...
        val item : Size = sizes[p1]

        if(item != null){
            holder.size.text = item.size.toString() + " " + item.unit
        }
    }
}